  ...
  const timelineData = [
    {
      ...
      role: "Part-time MPhil in Computer Science",
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 max-w-6xl">
        <h1 className="h3 font-bold text-center tracking-tight mb-8">About</h1>

        {/* --- CONNECTING THE DOTS SECTION --- */}
        <div className="bg-gray-50 dark:bg-gray-900 rounded-2xl shadow-lg hover:shadow-xl p-6 md:p-8 flex flex-col md:flex-row items-center gap-8 mb-12 duration-300 transition-all">
          {/* Profile Picture */}
          <div className="flex flex-col items-center flex-shrink-0">
            <img 
              ...
              alt="Jacky Profile" 
              loading="lazy" 
              className="w-48 h-48 md:w-56 md:h-56 object-cover rounded-full border-4 border-teal-500/20 shadow-md hover:scale-105 transition-transform duration-300" 
          ...
          <div className="flex-1">
            <h3 className="text-2xl font-bold mb-4 text-teal-700 dark:text-teal-400 flex items-center gap-2">
              Connecting the Dots
            </h3>
            <p className="text-gray-800 dark:text-gray-200 leading-relaxed mb-4 text-base md:text-lg">
              I am Jacky, a Computer Engineering and Journalism graduate of CUHK, currently transitioning my technical trajectory toward Human-Computer Interaction (HCI) research.
            </p>
            <p className="text-gray-800 dark:text-gray-200 leading-relaxed text-base md:text-lg">
              My background combines technical expertise with communication skills, which allows me to approach problems from multiple perspectives. Being a full-time Graduate Engineer at MTR and a part-time Computer Science MPhil student at CityU makes me able to bridge the gap between industry and academia.
            </p>
          </div>
        </div>
            ...
            <div className="relative border-l border-gray-200 dark:border-gray-700 ml-4 md:ml-6 pl-6 md:pl-8 space-y-8">
              {timelineData.map((item, idx) => (
                <div key={idx} className="relative group">
                  {/* Icon Node */}
                  <span className="absolute -left-[41px] md:-left-[49px] top-1.5 flex items-center justify-center w-8 h-8 rounded-full bg-white dark:bg-gray-800 border-2 border-teal-500 shadow-md group-hover:bg-teal-50 dark:group-hover:bg-teal-900/30 transition-colors duration-300">
                    {item.type === "education" ? (
                      <FaGraduationCap className="text-teal-600 dark:text-teal-400 text-sm" />
                    ) : (
                  ...
                  <div className="bg-gray-50 dark:bg-gray-900/50 rounded-xl p-5 dark:border-gray-850 shadow-sm hover:shadow-md dark:hover:border border-gray-100 hover:scale-[1.01] transition-all duration-200">
                    <span className="inline-block bg-teal-50 dark:bg-teal-200 text-teal-700 dark:text-teal-350 text-xs font-semibold px-2.5 py-1 rounded-md mb-2">
                      {item.period}
                    </span>
            ...
            <div>
              ...
              <div className="space-y-4">
                {awards.map((award, idx) => (
                  <div 
                    key={idx} 
                    className=" bg-gray-50 dark:bg-gray-900/50 rounded-xl p-4 flex gap-4 dark:border-gray-850 shadow-sm hover:shadow-md dark:hover:border border-gray-100 hover:scale-[1.01] transition-all duration-200"
                  >
                    <div className="flex-shrink-0 flex items-center justify-center w-10 h-10 rounded-lg bg-teal-100 dark:bg-teal-900/40 text-teal-600 dark:text-teal-400">
                      {idx === 0 ? <FaTrophy className="text-lg" /> : <FaAward className="text-lg" />}
                    </div>
                    <div>
                      <h4 className="font-bold text-gray-900 dark:text-gray-100 leading-snug text-sm md:text-base">
                        {award.title}
                      </h4>
            ...
            <div>
              ...
              <div className="space-y-6">
                {competenceCategories.map((cat, catIdx) => (
                  <div 
                    key={catIdx} 
                    className=" bg-gray-50 dark:bg-gray-900/50 rounded-xl p-5 dark:border-gray-850 shadow-sm hover:shadow-md dark:hover:border border-gray-100 hover:scale-[1.01] transition-all duration-200"
                  >
                    ...
                    <div className="flex flex-wrap gap-2">
                      {cat.skills.map((skill, skillIdx) => (
                        <div 
                          key={skillIdx} 
                          className="flex items-center gap-1.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 px-3 py-1.5 rounded-lg text-xs md:text-sm font-medium hover:text-teal-600 dark:hover:text-teal-400 hover:border-teal-500/30 dark:hover:border-teal-500/30 hover:scale-105 transition-all duration-200"
                        >
                          <span className="text-gray-400 dark:text-gray-500 group-hover:text-teal-600 dark:group-hover:text-teal-400 text-sm">
                            {skill.icon}
                          </span>
